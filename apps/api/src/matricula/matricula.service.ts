...
export class MatriculaService {
  ...
  create(dto: CreateMatriculaDto): Matricula {
    const existe = this.matriculas.find(
      (m) => m.alunoId === dto.alunoId && m.disciplinaId === dto.disciplinaId,
    );

    if (existe) {
      throw new ConflictException('Aluno já matriculado nesta disciplina');
    }
  ...
  findDisciplinasByAluno(alunoId: number): Matricula[] {
    return this.matriculas.filter((m) => m.alunoId === alunoId);
  }

  // GET /disciplinas/:id/alunos
  findAlunosByDisciplina(disciplinaId: number): Matricula[] {
    return this.matriculas.filter((m) => m.disciplinaId === disciplinaId);
  }

  // GET /cursos/:id/alunos
  // TODO: ainda não há regra de negócio para vincular matrícula -> curso
